...
内置 **Fibocom FM350-GL**（USB RNDIS）时，构建脚本会固化拨号默认：`GTUSBMODE` **40**（PID `0e8d:7126`）、`ip_change_fm350` **/32 onlink**、`modem_support`/`suggest_pdp` 的 PDP/CID **1**、空 APN 默认 **CMNET**（移动；联通/电信请在 LuCI 改 APN）。AP3000M 另注入：`97-ap3000m-fm350` 纠偏、`ap3000m-fm350-enforce-mode40` + USB hotplug **禁止回到 41**、以及加强版看门狗（约 2 分钟检测、失败约 3 分钟再试、可连续两次软重启、清 AT 抢占）。RNDIS 假死无法从驱动根治，靠模式锁定 + 自动恢复。

已集成 **Docker**（`luci-app-dockerman` / `dockerd`）；建议在 **16G** 存储机型使用，容器数据放可写分区，勿塞满 rootfs。X86 配置暂未启用 Docker。默认 **`dockerd.iptables=0`**，由 fw4/nft 防火墙区管理，避免与 nftables 混用触发「检测到旧版规则」。
